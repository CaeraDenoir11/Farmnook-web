...
import {ValidFormat} from "../../utils/ValidFormat";
...
function AddHaulerButton({onAddHauler}) {
	...
	const handleSubmit = async () => {
		...
		if (!ValidFormat(formData, setErrors)) {
			return;
		}
		...
		<>
			<div className="group fixed bottom-4 md:bottom-6 right-4 md:right-6 flex justify-center items-center text-white text-xs md:text-sm font-bold">
				...
				>
					...
					<span className="text-[0px] group-hover:text-xs md:group-hover:text-sm duration-300 cursor-pointer">
						Add Hauler
					</span>
				</button>
			</div>
			...
			{isOpen && (
				<div className="fixed inset-0 backdrop-blur-sm bg-black/40 flex justify-center items-center p-2 md:p-4">
					<div className="bg-[#F5EFE6] p-3 md:p-6 rounded-xl md:rounded-2xl shadow-2xl w-full max-w-xs sm:max-w-md md:max-w-2xl lg:max-w-3xl transition-all duration-300">
						<h2 className="text-lg sm:text-xl md:text-2xl font-bold mb-3 md:mb-4 text-[#1A4D2E] text-center cursor-pointer">
							Add Hauler
						</h2>
						...
						<div className="flex justify-end mt-3 md:mt-4 space-x-2 md:space-x-4">
							<button
								...
								className="px-2 md:px-4 py-1 md:py-2 bg-gray-400 text-white rounded-lg text-xs md:text-sm cursor-pointer"
							>
								Cancel
							</button>
							<button
								...
								disabled={loading}
								className="px-2 md:px-4 py-1 md:py-2 bg-[#1A4D2E] text-white rounded-lg text-xs md:text-sm cursor-pointer"
							>
