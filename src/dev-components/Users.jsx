...
export default function Users() {
	...
	const {haulers, vehicles, loadingDetails} = useBusinessDetails(
		selectedBusiness ? selectedBusiness.id : null
	);
	...
	const handleDeleteConfirmed = async () => {
		...
		try {
			console.log(
				`Deleting user ${userEmail} (ID: ${userIdToDelete}) from Firestore 'users' collection...`
			);
			await deleteDoc(doc(db, "users", userIdToDelete));
			console.log(
				`User ${userIdToDelete} deleted from Firestore 'users' collection.`
			);
			...
			console.log(
				`Deleted ${vehicleSnapshot.docs.length} associated vehicles.`
			);

			alert(
				`Successfully deleted business admin ${userEmail} and all related data from Firestore.`
			);
		} catch (error) {
	...
	const handleRowClick = (business) => {
		if (!business || !business.id) {
			console.error(
				"Invalid business object passed to handleRowClick:",
				business
			);
			return;
		}
	...
	if (loadingUsers || loadingSubscriptions) {
		return (
			<div className="flex-1 min-h-screen p-3 sm:p-5 bg-white flex justify-center items-center">
			...
			{!selectedBusiness ? (
				<>
					<div className="w-full max-w-6xl flex flex-col sm:flex-row justify-between items-center gap-2 mb-4">
						...
						<select
							...
							className="w-full sm:w-auto px-3 py-2 rounded-md border border-gray-300 text-sm shadow-sm focus:outline-none focus:ring-1 focus:ring-[#1A4D2E] cursor-pointer"
						>
				...
				<>
					<button
						...
						className="mb-4 self-start px-4 py-2 text-sm text-white bg-[#1A4D2E] rounded-md cursor-pointer hover:bg-gray-500"
					>
					...
					<div className="flex gap-4 mb-4">
						<button
							...
							className={`px-4 py-2 rounded-md ${
								viewMode === "Haulers"
									? "bg-[#1A4D2E] text-white"
									: "bg-gray-200 text-gray-800 cursor-pointer hover:bg-gray-500"
							}`}
						>
							Haulers
						</button>
				...
				<div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
					<div className="bg-white p-6 rounded-md shadow-xl max-w-md w-full text-center">
						...
						<p className="mb-4">
							...
							<strong>{userToDelete?.email}</strong> and all their associated
							haulers and vehicles from the database? This action cannot be
							undone.
						</p>
